...
d >= 5 #d 백터의 값이 5보다 크니?
d [d>5] # 값 중 5보다 큰 값만 출력
sum(d>5) # 5보다 큰 값의 개수 (d 백터에 5보다 큰 값은 총 4개이다다)
sum(d[d>5]) #5보다 큰 값의 합 (d백터에서 5보다 큰 숫자의 합은 30)
d==5 #백터 값 중 5와 같은면 true, 아니면 false로 출력 
condi <- d > 5 & d < 8 #dondi라는 값은 5~8사이 값으로 넣는다 
condi # 5보다 크고 8보다 작은 수만 true
d[condi] # 6 ,7 출력 돼